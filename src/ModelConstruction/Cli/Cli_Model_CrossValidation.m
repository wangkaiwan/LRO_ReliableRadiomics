...
min_model=20;
pop_num=100;
it_num=10; % similar results when it_num=50
cross_num=5;
NA=20;
CS=500;

fea_num_str = [10,10,10,10,10]*5; %set pre feature selection number, not available for clinical model

lamda=0;
class = 2;
outputpath = '../../../data/Models/Cli/';
...
load('indice_5_times'); % fixed indice for all 5 times  of 5-fold-CV

%% 
test_record = zeros(6,5,5);
val_record = zeros(6,5,5);

scale = [1 1 1 1 1]*1.2; % minority sample upsampling rate
...
for repeat_index = 1:5
    ...
    for c=1:5
        ...
        CT_train_all_fea = [CT_train_pre,CT_train_post,CT_train_post-CT_train_pre];
        CT_Delta_fea = [CT_all_pre,CT_all_post,CT_all_post-CT_all_pre];
        ...
        PET_train_all_fea = [PET_train_pre,PET_train_post,PET_train_post-PET_train_pre];
        PET_Delta_fea = [PET_all_pre,PET_all_post,PET_all_post-PET_all_pre];
...
%             fprintf('Testing: ');
        ...
        save([outputpath, 'Cli_Model_CV_',num2str(repeat_index),'_Fold_',num2str(c),'.mat']); 
    ...
    save([outputpath, 'Cli_Model_CV_',num2str(repeat_index),'.mat'],'test_record','val_record');
end