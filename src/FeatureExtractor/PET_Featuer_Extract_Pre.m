...
for i = 1:fileNum
    ...
    if isempty(pre_record{i,6}) || pre_record{i,6}==0
        ...
        load([dataPath,'\',folder{index},'\',fileName,'\PET\image\oriImg_Pre.mat']);
        load([dataPath,'\',folder{index},'\',fileName,'\PET\RTS\oriContour_Pre.mat']);
        ...
        dicomPath = ['..\..\original data\',folder{index},'\Pre\',fileName,'\PET'];
...
%              Texture_feature=extracting_texture_feature(cube);
            ...
            Texture_feature = [];
            voxel_size = [1,4]; % target voxel size in mm
            quantization = {'Uniform'};
            GrayLevel = [8,16,64,128]; % target gray level
...
fclose(fid);
save([dataPath,'\',folder{index},'\','PET_Pre_Feature_Record.mat'],'record');
save([dataPath,'\',folder{index},'\','PET_Pre_Feature.mat'],'fin_fea');
end