...
%Energy, Entropy, Correlation, Contrast,Homogeneity, Variance, SumMean, Inertia, 
...
featureVector = zeros(size(data,4),noDirections*size(distance,1)*...
    numHarFeature);
% featureVector = zeros(size(data,4),size(distance,1)*numHarFeature);
...
non_zero_min=maxImage;
for n1=1:size(I,1)
    for n2=1:size(I,2)
        for n3=1:size(I,3)
            if I(n1,n2,n3)~=0 && I(n1,n2,n3)<non_zero_min
                non_zero_min=I(n1,n2,n3);
            end
        end
    end
end
tempShift = double(maxImage-non_zero_min)/double(numLevels);
% I = floor(double(I)/double(tempShift));
for n1=1:size(I,1)
    for n2=1:size(I,2)
        for n3=1:size(I,3)
            if I(n1,n2,n3)~=0
               I(n1,n2,n3)= floor(double(I(n1,n2,n3)-non_zero_min)/double(tempShift));
               I(n1,n2,n3)=I(n1,n2,n3)+1;
            end
        end
    end
end
...
for direction =1:noDirections %currently 13 (for the 3d image)
    sliceMat=zeros(numLevels,numLevels);
            ...
            for tempCount = 1:size(rowj,1) 
                ...
                if rowT <= I1 && colT <= I2 && sliceT <= I3
                    if rowT > 0 && colT > 0 && sliceT > 0
                        ...
                        %Error checking for zeros, NANs and Infinite numbers
...
%                         end
    ...
    sliceMat=sliceMat+tempMat(:,:,slicej);
    end
%     for slicej =1:size(I,3)
    coMat(:,:,direction)= sliceMat;
%     end
end
% coMat=zeros(numLevels,numLevels);
% for direction =1:noDirections
%     coMat=coMat+Mat(:,:,direction);
% end
...
% for iteration=1:size(coMat,3)
%     pij=sum(sum(coMat)); 
% end
% pij=sum(pij);
Mat=zeros(numLevels,numLevels);
for iteration = 1:size(coMat,3) %directions
    Mat=Mat+coMat(:,:,iteration);
end
...
%     for m=1:numLevels
%         for n=1:numLevels
%             totalValue=totalValue+coMat(m,n,iteration);
%         end
%     end
    for j=1:numLevels
        for i=1:numLevels
