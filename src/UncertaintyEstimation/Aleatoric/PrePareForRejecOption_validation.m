...
outputpath = '../../../data/AleatoricUncertainty/ValAugmentation/';
repeat = 100;
...
for it = 1:repeat
    for CV = 1:5
        ...
        for wInd = 4:4
            ...
            for i = 1:crossNum
                ...
                [SEN_Cli,SPE_Cli,ACC_Cli,AUC_Cli,P_Cli] = Cli_OptimalSetFusion_val(CliName,wInd,lamda);
                [SEN_PET,SPE_PET,ACC_PET,AUC_PET,P_PET] = PET_OptimalSetFusion_val(PETName,wInd,lamda);
                [SEN_CT,SPE_CT,ACC_CT,AUC_CT,P_CT] = CT_OptimalSetFusion_val(CTName,wInd,lamda);

                pareto = [SEN_Cli, SPE_Cli; SEN_PET, SPE_PET; SEN_CT, SPE_CT];
                AUCs = [AUC_Cli AUC_PET AUC_CT];

                load(CTName,'PET_ind_val_data');
                ...
                [SEN,SPE,ACC,AUC,p_value]=cal_evaluation(P_fin,PET_ind_val_data);
                ...
                final_ind=[final_ind; PET_ind_val_data];
                result(i,:,CV) = [SEN,SPE,ACC,AUC,p_value];

                P_val{CV,i} = P_fin;
                Ind_val{CV,i} = PET_ind_val_data;
            end
    ...
    save([outputpath, 'PrepareForRejectOption_val_',num2str(it),'.mat'],'P_val','result','Ind_val');
end
