function [SEN,SPE,ACC,AUC,P_fin] = PET_OptimalSetFusion_val(FileName,wInd,lamda)
...
%     min_model = 10;
    [SEN,SPE,ACC,AUC] = PET_MyValdationAnalysis(wInd,Epal,EPOP,E_AUC,E_model,lamda,min_model,class,cli_sam_val_data,PET_sam_val_data,CT_sam_val_data,PET_ind_val_data,cli_n,pet_n,ct_n);
    ...
    P_fin=zeros(length(PET_ind_val_data),class);
    for j=1:length(PET_ind_val_data)
...
%         r=cal_reliability(sam_train_data,ind_train_data,sam_test_data(j,:),10,fin_POP,fin_model);
        ...
        temp_cli = cli_sam_val_data(j,:);
%         temp_pet = PET_sam_val_data(j,:)+0.025*randn(size(PET_sam_val_data(j,:)));
%         temp_ct = CT_sam_val_data(j,:)+0.025*randn(size(CT_sam_val_data(j,:)));
        temp_pet = PET_sam_val_data(j,:)+0.01*randn(size(PET_sam_val_data(j,:)));
        temp_ct = CT_sam_val_data(j,:)+0.01*randn(size(CT_sam_val_data(j,:)));
        p=PET_cal_single_probability(temp_cli,temp_pet,temp_ct,fin_POP,fin_model,cli_n,pet_n,ct_n);
